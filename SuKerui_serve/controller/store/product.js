...
router.post('/addSpu', (req, res) => {
    ...
    // ];
    let query = [{
            sql: "select id from t_category where  `name`=?",
            param: [spec]
        },
        {
            sql: "select id from t_brand where  `name`=?",
            param: [brand]
        },
        {
            sql: "select id from t_spec_group where  `name`=?",
            param: [spec]
        }
    ];
    ...
    let insert = "insert into t_brand_category (category_id,brand_id) values (?,?) on duplicate key update category_id=?,brand_id=?"
