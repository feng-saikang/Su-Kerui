        ...
        <a-card-meta title=" ">
          ...
          <template slot="description">
            <div class="price">
              ...
              <span class="percent"
                >{{ parseInt((item.special_price / item.price) * 100) }}%</span
              >
            </div>
          </template>
        </a-card-meta>
...
import { getTypeOneList } from "@/api/shopList/index";
export default {
  components: { Title },
  data() {
    return {
  ...
  methods: {
    async getTypeOneList_() {
      getTypeOneList("服饰").then(res => {
        console.log(res);
        let resulit = res.res;
        this.wantneedList = resulit.slice(200, 208);
      });
    },
  },
};
